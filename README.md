...
The compose file exposes ports `4096` and `3000`, mounts a persistent volume at `/home/agent`, and drops all capabilities except `SYS_PTRACE`, `NET_RAW`, `NET_ADMIN`, `SETUID`, `SETGID`, `SYS_CHROOT`, and `DAC_OVERRIDE`. Each granted capability and disabled security feature is documented inline in `docker-compose.yml`.
