...
> **Note**: The compose file requires `/dev/fuse` and `/dev/net/tun` device access, `security_opt: label=disable` (SELinux), and `security_opt: seccomp=unconfined` (user namespace creation for rootless Podman). See `docker-compose.yml`.
