...
> **Note**: The compose file requires `/dev/fuse` device access, `SETUID`/`SETGID` capabilities (for `newuidmap`/`newgidmap`), `security_opt: label=disable` (SELinux), and `security_opt: seccomp=unconfined` (user namespace creation for rootless Podman). See `docker-compose.yml`.
